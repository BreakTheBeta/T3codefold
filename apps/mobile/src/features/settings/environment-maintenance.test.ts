    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockImplementation(async () =>
          Response.json([
            { tag_name: "fold-server-v2.0.0-nightly.20260923.1" },
            { tag_name: "fold-server-v1.2.0", draft: true },
            { tag_name: "fold-server-v1.1.0" },
          ]),
        ),
    );
  ...
  it("walks release pages to find the host's channel", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        Response.json(Array.from({ length: 100 }, () => ({ tag_name: "fold-server-v2.0.0" }))),
      )
      .mockResolvedValueOnce(
        Response.json([{ tag_name: "fold-server-v1.0.0-preview.20260923.2" }]),
      );
