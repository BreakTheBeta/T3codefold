...
/** Provider updates contain cumulative text. Deliver only stable markdown boundaries,
 * independently for each message representation, and always forward final snapshots. */
export function makeAssistantDelivery(mode: ResponseStreamingMode) {
  const delivered = new Map<string, { length: number; at: number }>();
  return (event: ProviderAdapterV2Event, now: number): ProviderAdapterV2Event | null => {
    ...
    const offset = previous && previous.length <= message.text.length ? previous.length : 0;
    const { ready, rest } = splitBufferedAssistantText(message.text.slice(offset));
    const text = rest.length > 32_000 ? message.text : message.text.slice(0, offset + ready.length);
    if (!text.trim() || text.length === previous?.length) return null;
    delivered.set(key, { length: text.length, at: now });
    if (event.type === "message.updated") return { ...event, message: { ...event.message, text } };
    if (event.type === "turn_item.updated" && event.turnItem.type === "assistant_message")
      return { ...event, turnItem: { ...event.turnItem, text } };
    return event;
  };
}
