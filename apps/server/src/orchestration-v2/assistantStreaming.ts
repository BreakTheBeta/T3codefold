...
export function makeAssistantStreamingFilter(mode: ResponseStreamingMode) {
  ...
  return (event: ProviderAdapterV2Event, now: number): ProviderAdapterV2Event | null => {
    ...
    const offset = previous && message.text.startsWith(previous.text) ? previous.text.length : 0;
    const { ready: boundary, rest } = splitBufferedAssistantText(message.text.slice(offset));
    const ready =
      rest.length > 32_000 ? message.text : message.text.slice(0, offset + boundary.length);
    if (!ready.trim() || ready === previous?.text) return null;
