...
export function decide(
  ...
  if (userActions.includes(action.type) && !user)
    fail("Only the user can change pitboss authority or limits.", "forbidden");
  if (!["report", "submit"].includes(action.type) && !manager)
    fail("Only the current pitboss or user can manage work.", "forbidden");
  ...
  if (action.type === "pause" || action.type === "brief") {
    if (!state.role) return fail("Elect a pitboss first.");
    return {
      ...next,
      role:
        action.type === "pause"
  ...
  if (!user && manager && !state.role?.brief.projectIds.includes(existing.projectId))
    fail("Project is outside the current pitboss brief.", "forbidden");
