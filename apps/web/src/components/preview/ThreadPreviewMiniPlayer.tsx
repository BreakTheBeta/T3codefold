...
export function ThreadPreviewMiniPlayer({ threadRef, tabId, bottomInset }: Props) {
  ...
  const [detailsCardRect, setDetailsCardRect] = useState<{ right: number; bottom: number } | null>(
    null,
  );
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const nextCard = rect
        ...
        : null;
      setDetailsCardRect((current) =>
        current?.right === nextCard?.right && current?.bottom === nextCard?.bottom
          ? current
          : nextCard,
      );
