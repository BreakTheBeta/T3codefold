...
Persist report identities and task/project references before waking the recipient. Each machine-generated wake names the new message, ready task or stopped attempt, its current owner and the next management action; stable role rules stay in injected context instead of being repeated as a generic user message. Durable outbox keys record each obligation independently, so retries and restarts preserve delivery without replaying an old obligation when an unrelated revision or run changes. Reading a message does not consume it; acknowledgement follows a handled obligation. Reassignment changes routing without duplicating unresolved reports, and stale managers cannot mutate work after their authority is revoked. A lead waiting for input should sleep until that input changes instead of repeatedly asking whether work is ready.
...
Each environment retains its own GLaDOS and local project leads. This local delegation introduces no global boss. A local lead may coordinate a mirrored task only after GLaDOS explicitly transfers that task and an existing two-sided peer agreement names this environment as coordinator and fixes another environment as task home. Lead ownership remains local metadata; execution, workspace, provider selection, task revision and acceptance remain at the fixed home. Losing a peer never grants takeover. Moving a lead itself or delegating broader cross-environment project authority requires a separate design and the existing explicit user approval boundary.
...
The first implementation adds local lead creation and dormancy, generation-scoped management, task reassignment, versioned project context, owner-routed messages, shared worker admission, one automatic lead turn at a time and native client inspection. Reassignment can include a mirrored task within an already-approved fixed-home peer scope; it does not create remote authority or move the lead. The project context is a bounded current document retained through journal history; structured decision dependencies, granular retrieval and automatic stale-decision invalidation remain future work. Leads must explicitly amend task criteria when a decision changes.
...
The work subsystem enforces management and evidence authorship at its command interface and gives coordinators an explicit delegation-only role context. It cannot remove provider-native shell or computer-use tools from an already running coordinator session. Those tools share the session's user-selected runtime permissions, so this boundary is role guidance plus work-command authorization rather than a process sandbox. Hands-on execution remains auditable only when it is assigned to a tracked worker and returned as task evidence.
