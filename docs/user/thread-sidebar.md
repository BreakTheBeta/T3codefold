...
Routine worker questions and operational blockers stay with GLaDOS; they remain visible under **Working**. **Needs you** highlights decisions and prepared setup that require your review. When GLaDOS needs your judgment, open the task’s decision and choose an option or give your own direction. You can also answer in the GLaDOS conversation with the exact phrase it offers: `Choose <option> for task <task-id>`. Typed and transcribed voice messages use the same path. Only the complete explicit reply from your authenticated web or mobile conversation resolves that pending decision; unrelated text and agent messages do not. Only that task and its dependencies wait; independent work continues. You can leave a decision for later. Acknowledging a message does not approve it. After your answer, GLaDOS or the project lead reconciles the retained work within the existing brief and limits.
...
With automatic verification enabled, GLaDOS inspects the project and saves its checks before assigning new work. If manual review is selected, or proof requirements would change after an attempt, GLaDOS prepares a proposal. Review the readiness, checks and permitted effects in the conversation or under **Review proposed settings**. Then use **Save and select profile**, or reply `Approve verification for task <task-id>` in the GLaDOS conversation. The conversation reply is bound to the linked decision and exact stored proposal version and digest; a changed proposal needs a fresh decision. Saying “continue,” bare approval, silence, and unrelated text do not approve configuration. Saving retains other pending decisions and never substitutes a missing tool or device with a passing result.
