...
If verification setup is missing, GLaDOS can propose concrete commands for review. Open the task’s **Review proposed settings**, inspect readiness, checks and permitted effects, then **Save and select profile**. Only saving approves configuration; saying “continue” in chat does not. Saving retains pending decisions and never substitutes a missing tool or device with a passing result.
