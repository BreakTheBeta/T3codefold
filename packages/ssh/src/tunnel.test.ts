...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(
      script,
      "could not install 'https://github.com/BreakTheBeta/T3codefold/releases/download/fold-server-latest/t3.tgz'",
    );
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --prefer-online --package 'https://github.com/BreakTheBeta/T3codefold/releases/download/fold-server-latest/t3.tgz'",
    );
    assert.include(
      script,
      "require_installed_t3_cli npm exec --yes --prefer-online --package 'https://github.com/BreakTheBeta/T3codefold/releases/download/fold-server-latest/t3.tgz'",
    );
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    ...
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --prefer-online --package 't3@nightly; touch /tmp/t3-owned'",
    );
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "t3@nightly" }),
      "https://github.com/BreakTheBeta/T3codefold/releases/download/fold-server-nightly/t3.tgz",
    );
    assert.include(
