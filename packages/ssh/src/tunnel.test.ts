...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/BreakTheBeta/T3codefold/releases/download'",
    );
    assert.include(script, 'T3_RUNTIME_DIR="$HOME/.t3/runtime/fold/versions/$T3_ARCHIVE_VERSION"');
...
describe("archive runner script", () => {
  ...
  const makeMirror = Effect.fn("makeMirror")(function* (root: string) {
    ...
    const release = `${root}/mirror/fold-server-v${archiveVersion}`;
    const script = [
      "set -eu",
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const versionsDir = `${home}/.t3/runtime/fold/versions`;
