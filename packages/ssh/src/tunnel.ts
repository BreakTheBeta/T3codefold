...
if ! t3_runtime_ready; then
  ...
  # under it.
  T3_LOCK="$HOME/.t3/runtime/fold/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_STAGING="$(mktemp -d "$HOME/.t3/runtime/fold/versions/.staging-XXXXXX")"
  ...
  t3_fetch "$T3_RELEASE_BASE_URL/fold-server-v$T3_ARCHIVE_VERSION/SHA256SUMS" "$T3_STAGING/SHA256SUMS" @@T3_ARCHIVE_CHECKSUMS_SECONDS@@
  t3_fetch "$T3_RELEASE_BASE_URL/fold-server-v$T3_ARCHIVE_VERSION/$T3_ARCHIVE" "$T3_STAGING/$T3_ARCHIVE" @@T3_ARCHIVE_DOWNLOAD_SECONDS@@
