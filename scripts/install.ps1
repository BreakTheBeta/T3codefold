...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    Step "Downloading..."
    try {
      Fetch "$baseUrl/fold-server-v$version/SHA256SUMS" (Join-Path $staging "SHA256SUMS")
    } catch {
    ...
    Fetch "$baseUrl/fold-server-v$version/$archive" (Join-Path $staging $archive) -progress
