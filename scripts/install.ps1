...
if (-not $version) {
  ...
  $tagPattern = switch ($channel) {
    "stable" { '^fold-server-v\d+\.\d+\.\d+$' }
    "nightly" { '^fold-server-v\d+\.\d+\.\d+-nightly\.\d+\.\d+$' }
    "preview" { '^fold-server-v\d+\.\d+\.\d+-preview\.\d+\.\d+$' }
  ...
  $version = $tag.Substring("fold-server-v".Length)
}
...
$archive = "$stem.zip"
$versionsDir = Join-Path $t3Home "runtime\fold\versions"
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    try {
      Invoke-WebRequest -Uri "$baseUrl/fold-server-v$version/SHA256SUMS" -OutFile (Join-Path $staging "SHA256SUMS") -UseBasicParsing
    } catch {
      $status = $_.Exception.Response.StatusCode.value__
      if ($status -eq 404) {
        Fail "t3 $version has no release archive for win32-$arch; releases before the self-contained CLI can only be installed with 'download the npm tarball from the Fold release page'"
      }
      throw
    }
    Invoke-WebRequest -Uri "$baseUrl/fold-server-v$version/$archive" -OutFile (Join-Path $staging $archive) -UseBasicParsing
